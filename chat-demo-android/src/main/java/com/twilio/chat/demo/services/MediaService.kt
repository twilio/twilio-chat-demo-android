    ...
    private fun upload(intent: Intent) {
        ...
        launch(coroutineContext) {
            ...
            try {
                if (cursor.moveToFirst()) {
                    ...
                    val options = Message.options()
                            ...
                            .withMediaProgressListener(object : ProgressListener() {
                                override fun onStarted() = Timber.d("Start media upload")
                                override fun onProgress(bytes: Long) = Timber.d("Media upload progress - bytes done: ${bytes}")
                                override fun onCompleted(mediaSid: String) = Timber.d("Media upload completed")
                            })
                    ...
                    TwilioApplication.instance.basicClient.chatClient?.channels?.getChannel(channel.sid, ChatCallbackListener<Channel> {
                        it.messages.sendMessage(options, ChatCallbackListener<Message> {
                            Timber.d("Media message sent - sid: ${it.sid}, type: ${it.type}")
                ...
                Timber.e("Failed -> message: ${e.message}")
            ...
            channel.messages.getMessageByIndex(messageIndex, ChatCallbackListener<Message> { message ->
                ...
                try {
                    ...
                    media.download(outStream, ChatStatusListener { Timber.d("Download completed") }, object : ProgressListener() {
                        ...
                        override fun onProgress(bytes: Long) = Timber.d("Media download progress - bytes done: ${bytes}")
                        override fun onCompleted(mediaSid: String) {
                            Timber.d("Media download completed")
                            deferred.complete(mediaSid)
                        }
                    })

                } catch (e: Exception) {
                    Timber.d("Failed to download media - error: ${e.message}")
                    deferred.completeExceptionally(e)
                }
            })
